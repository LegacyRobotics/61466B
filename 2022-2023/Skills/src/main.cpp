...
int drivePID(){
  ...
  while(enableDrivePID){
    ...
    std::cout <<  averagePosition << "," << Gyro.rotation() << ","  << desiredValue << "," << desiredTurnValue << "\n";
    vex::task::sleep(20);
...
int data(){
  while(true){
    float avgpos = (Lfront.rotation(degrees)+Rfront.rotation(degrees))/2;
    if (desiredValue > avgpos){
      Lfront.spin(forward,12,volt);
      Lback.spin(forward,12,volt);
      Rfront.spin(forward,12,volt);
      Rback.spin(forward,12,volt);
    } else {
      Lfront.stop();
      Lback.stop();
      Rback.stop();
...
float error = //Sensor - target;
totalError += error;
float derivative = error - previousError;

float motorPower = (error * kP +  derivative * kD); //Add values up
...
int marker;
vex::task PIDController(drivePID);
...
desiredValue = -185;
wait(0.5, seconds);
Intake.spinFor(reverse, 0.7,turns);
